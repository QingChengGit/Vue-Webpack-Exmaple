    ...
    <div class="yunnex-dialog-mask" v-if="dialogConfig.isShow">
        <div class="yunnex-dialog" :class="dialogConfig.dialogClass">
            <div :class="titleClass">
                {{title}}
                <i v-if="dialogConfig.closeClass" :class="dialogConfig.closeClass" @click="closeDialog"></i>
            </div>
            <!-- 对换框的内容区内容,当内容模板中的标签含有ok-btn、cancel-btn则分别表示此两标签代表确定、取消按钮的功能 -->
...
<script>
    module.exports = {
        ...
        methods: {
            ...
            close: function(flag) {
                this.$emit('dialog-closing', flag);
            }
        },
        mounted: function() {
            var dialog = document.querySelector('.yunnex-dialog'),
